create database restauranteProva;

use restauranteProva;
...
-- Como alterar o tipo de dado de um campo. Float no MySQL não aceita valores grandes
alter table ingredientes MODIFY valor_unitario double;
alter table pratos MODIFY valor_unitario double;
alter table itens_encomenda MODIFY valor_unitario double;
alter table itens_compra MODIFY valor_unitario double;
...
insert into unidades (descricao, sigla) values ('Quilo','kg');
insert into unidades (descricao, sigla) values ('Lata', 'Lt');
insert into unidades (descricao, sigla) values ('Unidade', 'Und');
...
insert into usuarios (nome_completo,login,senha) values ('André Mendes','andre',MD5('123'));

create table motoboys (
	cod_motoboy int not null auto_increment primary key,
	nome varchar(100),
